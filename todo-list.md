...
| BBS-01 | Steel from separate BBS models (BBS Steel tab + sheet) | — | **done** (`v1.42.0` `lib/bbs_steel_engine.py`, `lib/bbs_steel_tab.py`, `read_bbs_model`; UMA NIWAS's 20 BBS models read live in Revit 2025 - 132.38 t; the dialog's Read run live in `v1.47.1`) |
...
- **P8** is the one that is still open in code. `v1.38.0` moved the Site Items, Rate Analysis,
  ...
  `script.py` went from 7,082 to **5,508 lines**. What is left is mostly Revit-reading code (which
  must stay out of `lib/`), the export orchestration and the per-category wiring loops.
...
**Slice 3 (`v1.48.0`), by the same rule - move the decision, keep the read:**
- `structural_material_candidates` / `resolve_structural_material` went to `lib/parameter_engine.py`,
  with the value reader handed in.
- The Slab/Foundation filter went to `lib/rule_engine.py` as `filter_logical_elements`, with the
  three classifiers handed in.
- The choice of beam for a column-hosted bar went to `lib/rebar_engine.py` as
  `choose_beam_for_bar`; the box and curve reads stay.
- Each was compared with `HEAD` on random inputs: 4,000 beam, 3,000 filter and 3,000 material
  cases, with 0 differences.
