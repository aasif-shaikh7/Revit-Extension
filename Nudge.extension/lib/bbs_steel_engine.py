...
def needs_reading(entry, signature=False):
    """True for a model that is unread, changed, failed last time, or was
    read before its bars were kept."""
...
def _plain(value):
    """A value the store can hold: a number, or text."""
    if value is None:
        return u""
