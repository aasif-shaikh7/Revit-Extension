...
# P12: the rate database shown in the Rate Database tab, with the same
# ready guard - a headless export never loads the tab and must not save
# its empty list over the owner's rates.
rate_db_state = []
rate_db_ready = [False]
...
try:
    ...
    if not os.path.exists(xaml_path):
        ...
        def capture_and_save_settings():
            ...
            # P12: the rates and this project's location, under the same
            # guard - only once the tab has loaded them.
            try:
                if rate_db_ready[0]:
                    from rate_database_engine import (
                        save_rate_database, set_project_location)
                    settings = save_rate_database(settings, rate_db_state)
                    location_box = window.FindName("RateDbProjectLocation")
                    if location_box is not None:
                        settings = set_project_location(
                            settings, safe_text(doc.Title, ""),
                            location_box.Text)
            except:
                pass
